...
function InterviewForm(props) {
  ...
  if (redirect) {
    return <Redirect to="/viewTrainees" />;
  }
  return (
        ...
        <form id="interview-form" onSubmit={e => handleSubmit(e)}>
          ...
          <span className="w-100">
            ...
            <datalist id="consultants">
              {consultants.map(({ employeeID, firstName, lastName }) => (
                <option key={employeeID} value={`${employeeID}: ${firstName} ${lastName}`} />
              ))}
            </datalist>
          </span>
